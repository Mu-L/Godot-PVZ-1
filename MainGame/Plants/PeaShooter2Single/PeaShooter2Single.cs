...
public partial class PeaShooter2Single : Plants
{
	...
	async public void Shoot()
	{
		...
		//GD.Print(bullet);
		bullet.Position = GetNode<Node2D>("./Head/Idle_mouth").Position + new Vector2(10, -6.5f); // 设置子弹位置为头部的嘴部
		bullet.ShadowPositionY = GetNode<Node2D>("Shadow").GlobalPosition.Y; // 设置子弹阴影位置为阴影的全局位置
		AddChild(bullet); // 添加子弹到场景中
		//GD.Print(bullet);
		bullet.Position = GetNode<Node2D>("./Head/Idle_mouth").Position + new Vector2(5, -6.5f); // 设置子弹位置为头部的嘴部
		bullet.ShadowPositionY = GetNode<Node2D>("Shadow").GlobalPosition.Y; // 设置子弹阴影位置为阴影的全局位置
		AddChild(bullet); // 添加子弹到场景中
		//GD.Print(bullet);
		bullet.Position = GetNode<Node2D>("./Head/Idle_mouth").Position + new Vector2(0, -6.5f); // 设置子弹位置为头部的嘴部
		bullet.ShadowPositionY = GetNode<Node2D>("Shadow").GlobalPosition.Y; // 设置子弹阴影位置为阴影的全局位置
		AddChild(bullet); // 添加子弹到场景中
